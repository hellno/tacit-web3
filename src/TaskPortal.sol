...
contract TaskPortal {
    ...
    event NewNodeCreated(Node node);
    ...
    function addNode(bytes32 _name, bytes32 _parent, bytes32 _data, address _creator, NodeType _nodeType) private {
        ...
        emit NewNodeCreated(nodes[path]);
    }


    function addTask(string memory title, string memory description) public {
        bytes32 name = keccak256(abi.encode(title));
        addNode(name, "", keccak256(abi.encode(description)), msg.sender, NodeType.Task);
    }
